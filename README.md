...
<p>このリポジトリは、readコマンドを実行して指定した変数に代入しechoコマンドを使用して内容を表示します</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、readコマンドを実行して変数を取得して表示してみてください</p>
...
<div class="wp-block-group">
<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>read -p "username: " username
read -s -p "password: " password
echo "username:""$username"" password:""$password"</code></pre>



<details class="wp-block-details"><summary>出力結果</summary>
<pre class="wp-block-code has-background" style="background-color:#ffeeee"><code>username:user password:admin</code></pre>
</details>
</div>
...
<pre class="wp-block-code has-24292-eff-color has-text-color has-background has-1-125-rem-font-size" style="background-color:#ffeeee"><code>username:user password:admin</code></pre>
</details>
</div>
</details>
...
<div class="wp-block-group">
<p>read -pのように-pオプションをつけることで入力プロンプトを表示できるようになります<br>read -sの-sは入力中に文字を表示させないようにします<br>今回の例では末尾に記述したusernameとpasswordが代入される変数になり、echoコマンドで変数を指定することでreadで取得したユーザー入力を確認することができます</p>
</div>
